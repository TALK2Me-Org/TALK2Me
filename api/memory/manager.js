 ...
 * 
 * Wykorzystuje:
 * - OpenAI Embeddings do tworzenia wektorów semantycznych (1536D)
 ...
 * 
 * Główne metody:
 * - saveMemory() - zapisuje wspomnienie z embedding
 * - getRelevantMemories() - similarity search
 * - extractEntities() - wyciąga imiona, daty, relacje
 * - formatMemoriesForContext() - formatuje dla AI
 * 
 * Uwaga: Wymaga OpenAI API key w konfiguracji
 */
...
class MemoryManager {
  ...
  async initialize() {
    ...
    try {
      ...
      // Sprawdź czy tabela memories_v2 istnieje i ma poprawną strukturę
      console.log('🔍 Testing memories_v2 table access...')
      const { data, error } = await this.supabase
        .from('memories_v2')
        .select('id, user_id, summary, embedding, memory_type, importance')
        .limit(1);
      
      console.log('📊 Table test result:', { data, error })
      
      if (error) {
        console.error('❌ memories_v2 table access error:', error);
        if (error.code === '42P01') {
          console.error('Table does not exist. Please run the migration.');
        } else {
          console.error('Table exists but has access/structure issues:', error.message);
        }
        this.enabled = false;
      } else {
  ...
  async getRelevantMemories(userId, query, limit = 5, threshold = 0.7) {
    ...
    try {
      ...
      // Search for similar memories using memories_v2 function
  ...
  async getMemoriesByType(userId, memoryType, limit = 10) {
    try {
      const { data, error } = await this.supabase.rpc('get_memories_by_type_v2', {
        p_user_id: userId,
